...
const itemLista = document.getElementById("articulos-template");

function renderPage (urlRender, data) {

    const xhr = new XMLHttpRequest();
    xhr.open("GET", urlRender);

    xhr.send();
    xhr.addEventListener("load", () => {

        const plantilla = Handlebars.compile(xhr.response);
        itemLista.innerHTML = plantilla(data);
    });
}

function homePage () {
    return renderPage("../plantilla.hbs", { articulos });
}
...
selectDOM.addEventListener("change", () => {
    let html = "";
    if (selectDOM.value == "mayorAMenor") {
        ...
        for (let i = 0; i < articulos.length; i++) {
            ...
            html += `<div class="card">
                        <img src="${element.urlImagen}" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h2 class="card-title text-center">${element.nombre}</h2>
                            <h3 class="card-text text-center">${element.precio}</h3>
                        </div>
                    </div>`
        }

    } else if (selectDOM.value == "menorAMayor") {
        console.log("Select: " + selectDOM.value);
    ...
    return itemLista.innerHTML = html;
});

inputDOM.addEventListener("change", () => {
    let html = "";
    const valor = inputDOM.value;
    for (let i = 0; i < articulos.length; i++) {
        const element = articulos[i];
        if (element.nombre.toLowerCase().includes(valor.toLowerCase())) {
            html += `<div class="card">
                        <img src="${element.urlImagen}" class="card-img-top" alt="...">
                        <div class="card-body">
                            <h2 class="card-title text-center">${element.nombre}</h2>
